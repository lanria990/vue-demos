...
<template>
    <div class="container">
        ...
        <div class="row exclamations-viewer">
            ...
            <div class="col-md-4">
                <Exclamation-List title="All Exclamations "
                                  :user="user"
...
<script>
    ...
    export default {
        ...
        ]).then(([{data:meData},{data:exclamationsData}])=>{
            this.user = meData.user;
            this.exclamations = exclamationsData.exclamations;
        })
    },
    components:{
        ExclamationList,ExclamationSearchList, ExclamationAddForm,
    },
    methods:{
        onExclamationRemoved(id){
            axios.delete(`/api/exclamations/${id}`).then(()=>{
                this.exclamations = this.exclamations.filter(item=>item.id!==id);
            });
        },
